...
from our_policies.policy import HeuristicPolicy, DummyMemory
        ...
        if len(self.opp_agent_map.get(handle, [])) == 0:
            if self.switches.get(position, None) is None:
                self.shortest_distance_agent_map[(
                    handle, position[0], position[1])] = 1
...
class DeadLockAvoidanceAgent(HeuristicPolicy):
    ...
    def build_agent_position_map(self):
        ...
        self.agent_positions = np.zeros(
            (self.env.height, self.env.width), dtype=int) - 1
    ...
    def extract_agent_can_move(self):
        ...
        for handle in range(self.env.get_num_agents()):
            ...
            if agent.status < RailAgentStatus.DONE:
                next_step_ok = self.check_agent_can_move(handle,
                                                         ...
                                                         self.shortest_distance_walker.same_agent_map.get(
                                                             handle, []),
                                                         self.shortest_distance_walker.opp_agent_map.get(
                                                             handle, []),
                                                         full_shortest_distance_agent_map)
                if next_step_ok:
                    next_position, next_direction, action, _ = self.shortest_distance_walker.walk_one_step(
                        handle)
                    self.agent_can_move.update(
                        {handle: [next_position[0], next_position[1], next_direction, action]})
        ...
        if self.show_debug_plot:
            ...
            for handle in range(self.env.get_num_agents()):
                ...
                plt.imshow(
                    full_shortest_distance_agent_map[handle] + shortest_distance_agent_map[handle])
    ...
    def check_agent_can_move(self,
        ...
        for opp_a in opp_agents:
            ...
            delta = ((my_shortest_walking_path - opp -
                     agent_positions_map) > 0).astype(int)
